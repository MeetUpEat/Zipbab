...
interface MeetingRepository {
    ...
    suspend fun createMeeting(mainMenu: String): List<Meeting>

    suspend fun updateAttendanceCheckMeeting(meetingDocumentID: String)

    suspend fun endMeeting(meetingDocumentID: String)

    // 참여 대기중인 멤버리스트에 신청자 추가하기
    suspend fun addPendingMember(meetingDocumentID: String)

    // 참여 대기중인 멤버를 참여자 리스트로 옮겨주기
    suspend fun approveMember(meetingDocumentID: String)
